...
(function(global, $){
	...
	/**
	 * --------------------------------
	 * #move-focus img[role=button] 제어
	 * --------------------------------
	 */

	 $('#move-focus img[role=button]').on('keydown', function(e){
...
(function(global, $){
	...
	$layer_btn.on('click', function(){
		$layer_content_dim.filter('.layer_content')
			.attr('tabindex', 0)
			.focus();
	});
...
(function(global, $){
	...
	var infowindow = new daum.maps.InfoWindow({
		content : '<a href="http://kipfa.co.kr" style="position: relative; left: 14px; display: block; text-align: center; font-size: 12px; text-decoration: none; color: #838cd2;">한국 인터넷 전문가 협회</a>' // 인포윈도우에 표시할 내용
	});
	// // 인포윈도우를 지도에 표시한다
...
(function(global, $){
	...
	var $gif_checked = $('.gif_checked');

	$('#stop-gif-all button').on('click', function(e){
		e.preventDefault();
		var _this = this;
		if (_this.innerHTML === '이미지 정지') {
			$.each($gif_checked, function(index, el){
				var $item = $gif_checked.eq(index);
				$item.data('origin_src', $item.attr('src'));
				$item.attr('src', $item.data('stop-gif'));
				_this.innerHTML = '이미지 재생';
			});
		} else {
			$.each($gif_checked, function(index, el){
				var $item = $gif_checked.eq(index);
				$item.data('stop-gif', $item.attr('src'));
				$item.attr('src', $item.data('origin_src'));
				_this.innerHTML = '이미지 정지';
			});
		}
	});
